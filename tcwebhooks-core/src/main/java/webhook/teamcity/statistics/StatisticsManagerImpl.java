...
public class StatisticsManagerImpl implements StatisticsManager {
	...
	public void reportStatistics(LocalDateTime now) {
		...
		if (unreportedStats.size() >= this.myWebHookMainSettings.getReportStatisticsFrequency()) {
			try {
				WebHookConfig whc = new WebHookConfig("_Root", "_Root", "http://localhost:8111/webhooks/endpoint.html", Boolean.TRUE, new BuildState().enable(BuildStateEnum.REPORT_STATISTICS), "statistics-report", false, false, null, null);
